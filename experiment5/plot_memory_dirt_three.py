...
ax1.set_xlabel('Payload Size', labelpad=12, fontweight='bold')
ax1.set_ylabel('End-to-end Reset Latency (ms)', color=color_reset, fontweight='bold')
line1 = ax1.errorbar(
    x_indexes, grouped['reset_mean_ms'], yerr=grouped['reset_std_ms'],
    fmt='-o', color=color_reset, linewidth=2.5, elinewidth=1.5, capsize=4,
    label='Mean End-to-end Reset Latency ± Std', zorder=2
)
...
line2 = ax2.errorbar(
    ...
    label='Mean Function Execution ± Std', zorder=2
)
...
line3 = ax3.errorbar(
    ...
    label='Mean PCI Net Reset Latency ± Std', zorder=3
)
